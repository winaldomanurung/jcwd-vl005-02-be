  ...
  try {
    ...
    GET_PRODUCTS = `SELECT 
      ...
      p.stock, 
        ...
        p.unit
    FROM products p
    LEFT JOIN categories c ON p.category = c.id WHERE p.name LIKE '%${name}%' AND p.price >= ${lowPrice} AND p.price <= ${highPrice} AND c.name LIKE '%${category}%' ORDER BY ${sortBy} ${order} LIMIT ${offset}, ${limit} ;`;
    ...
    let total;
    if (name != "" || category != "" || parseInt(price) != 1) {
